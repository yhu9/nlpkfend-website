<html>
    ...
    <body>
        ...
        <?php
            ...
            //get field values for payment to add
            ...
            if($result !== false){
                ...
                //Initialize and create the add payment sql statement
                $sql1 = "INSERT INTO Payment (";
                $sql2 = "VALUES (";
                $is_first = 1;
                foreach($finfo as $field){
                    if($field->name != "paymentID"){
                ...
                //Create the combined sql statement and execute the addition of the new payment
                $sql = "$sql1 $sql2";
                $result = mysqli_query($db,$sql);

                //Check to make sure the INSERT statement executed
                if($result !== false){
                    echo "<h3 align=\"center\">Successfully added new payment!</h3>";

                    //get last inserted payment
                    $paymentData = getLastInsert($db,"Payment");
                    $accountData = getAccountByID($db,$paymentData['data'][0]['fk_accountID']);

                    //show payment data inserted
                    showAdvancedData($accountData['data'],$accountData['fields'],'Account Updated',"viewDetails.php");
                }else{
                    echo("sql statement: " .$sql);
                    echo "<br>";
                    echo("Could not add the new payment: <b>" .mysqli_error($db). "</b>");
                }
