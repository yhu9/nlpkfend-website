...
function attendance_basic($db){
    ...
    $sql2 = "WHERE room = 'green' and status='active'";
...
function attendance_other($db){
    ...
    $sql2 = "WHERE studentID = fk_studentID AND room <> 'green' AND room_name ='green' AND date = DATE(NOW()) ORDER BY last_name";
...
function attendance_employee($db){
    ...
    $sql2 = "WHERE employeeID = fk_employeeID AND room_name = 'Green' AND date = DATE(NOW()) ORDER BY last_name";
...
function getAttendance($db,$first_name,$last_name,$type){
    $sql = "SELECT verifier, MAX(time) as `time` FROM Attendance,Student WHERE studentID = fk_studentID AND first_name = \"$first_name\" AND last_name = \"$last_name\" AND type = \"$type\" AND room_name = \"Green\" AND date = DATE(NOW()) GROUP BY verifier ORDER BY `time` DESC";
...
function getEmployeeAttendance($db,$first_name,$last_name,$type){
    $sql = "SELECT MAX(time) as `time`,verifier FROM Employee_Attendance,Employee WHERE employeeID = fk_employeeID AND first_name = \"$first_name\" AND last_name = \"$last_name\" AND type = \"$type\" AND room_name = \"Green\" AND DATE(NOW()) = date GROUP BY verifier ORDER BY `time` DESC";
