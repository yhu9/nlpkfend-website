...
//get basic income info
function getIncomeBasic($db){
    $sql1 = "SELECT name,Income.* FROM Income";
    ...
    if($result !== false){
        ...
        echo "Error getting Income Data!<br>\n";
    }
...
//query the last insert from the income table
function getLastInsertData($db){
    $sql = "SELECT * FROM Income WHERE incomeID = LAST_INSERT_ID()";
    $result = mysqli_query($db,$sql);
    $data = array();
    if($result !== false){
        $fields = mysqli_fetch_fields($result);
...
//query distinct categories from the incomes
function queryDistinctCategories($db){
    $sql = "SELECT DISTINCT(category) FROM Income
            ORDER BY category";
...
//query distinct accounts from the incomes
function queryDistinctAccounts($db){
    $sql = "SELECT DISTINCT(bank_account) FROM Income
...
function queryCategoryAmount($db,$category,$month,$year){
    $sql = "SELECT DISTINCT(category), COALESCE(SUM(amount),0) as `total`
        FROM Income 
...
function queryAccountAmount($db,$account,$month,$year){
    $sql = "SELECT DISTINCT(bank_account), COALESCE(SUM(amount),0) as `total`
        FROM Income 
...
//query for getting the income report for a particular month and year
function queryDetailIncome($db,$category,$account,$month,$year){
    if($category != '')
        $sql = "SELECT * FROM Income WHERE YEAR(date) = $year AND MONTH(date) = $month AND category = '$category' ORDER BY date";
    elseif($account != '')
        $sql = "SELECT * FROM Income WHERE YEAR(date) = $year AND MONTH(date) = $month AND bank_account = '$account' ORDER BY date";
    else
    ...
    if($result !== false){
        ...
        echo "<h2>Error queryDetailIncome</h2><br>";
...
//show income Report
function showIncomeReportByAccount($db,$year){
    ...
    foreach($distinct_names['data'] as $row){
        ...
        //bank_account total
        echo "<th class='data'>$ $cat_total</th>";
        echo "\n</tr>";
    }
...
//show income Report
function showIncomeReportByCategory($db,$year){
    ...
    foreach($categories['data'] as $row){
        ...
        for($i = 0; $i < count($months); $i++){
            ...
            $mon_total[$i] += $amount;
            $m_count = $i + 1;
            echo "<td class='data' onclick=\"post('viewDetails.php',{category:'$category',month:$m_count,year:$year})\">\n";
            echo "$ "."$amount";
            echo "</td>";
        }
        //category total
...
function showDeleteableIncome($data,$fields){
...
//show the detailed income report for a particular month and year
function showDetails($db,$category,$account,$month,$year){
    $total = 0;

    $months = array('Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec');
    $income_data = queryDetailIncome($db,$category,$account,$month,$year);

    $found = count($income_data['data']);
    echo "<u>$found records $months[$month] $year</u><br>\n";
    echo "<form method='POST' action=''>";
    echo "<table class='data' align=\"center\">";
    echo "<tr>\n";
    foreach ($income_data['fields'] as $f){
        echo "<th>". str_replace('_',' ',$f->name) ."</th>\n";
    }
    echo "<th></th>";
    echo "<th></th>";
    ...
    foreach($income_data['data'] as $row){
        $total += $row['amount'];
        $id = $row[0];
        echo "<tr>\n";
        foreach($income_data['fields'] as $f){
