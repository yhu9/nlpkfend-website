        ...
        <?php
            ...
            if($result !== false){
                ...
                foreach($finfo as $field){
                    if($field->name != "attendanceID"){
                        ...
                        if(strpos($field->name,'date') !== false or $field->name == "DOB"){
                            ...
                            $sdata = getStudentByName($db,$first_name,$last_name);
                            $sid = $sdata['data'][0]['studentID'];
                            $val = "(SELECT tuition_type FROM Student WHERE studentID = $sid)";
                            ...
                            if(count($student_data['data']) == 1)
                            {
                                ...
                                $studentID = $student_data['data'][0]['studentID'];
                            }
                            $val = "".$studentID;
                        }
                        else
                        ...
                        if($val != "" and $val != "--"){
                            if($field->type == 16 OR $field->type == 1 OR $field->type == 2 OR $field->type == 3 OR
                                ...
                                $field->type == 246 OR $field->name == 'tuition_type' OR $field->name == 'auth_type'){
                                if($is_first == 1){
                                    $sql1 .= "$str_fieldname";
                                    $sql2 .= "$val";
                                    $is_first= 0;
                ...
                if($result !== false){
                    ...
                    $attendanceData = getLastInsert($db,"Attendance");
