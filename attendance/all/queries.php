...
function getAttendance($db,$id,$type){
    if($type == 'sign out')
        $sql = "SELECT verifier, MAX(time) as `time` FROM Attendance,Student WHERE studentID = fk_studentID AND studentID=$id AND type = \"$type\" AND date = DATE(NOW()) GROUP BY verifier ORDER BY `time` DESC";
    elseif($type == 'sign in')
        $sql = "SELECT verifier, MIN(time) as `time` FROM Attendance,Student WHERE studentID = fk_studentID AND studentID=$id AND type = \"$type\" AND date = DATE(NOW()) GROUP BY verifier ORDER BY `time`";
